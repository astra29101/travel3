...
const BookingConfirmationPage: React.FC = () => {
  ...
  const handleConfirm = async () => {
    if (!bookingData || !user) {
      setError('Missing booking data or user information.');
      return;
    }
    ...
    if (!isValidUUID(bookingData.packageId)) {
      setError('Invalid package ID format.');
      return;
    }

    if (!isValidUUID(bookingData.guideId)) {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'An error occurred while confirming your booking. Please try again.';
